...
public class Bank {
    ...
    public void mainMenu(User user) {
        while (true) {
            if(user.getRole().equalsIgnoreCase("ADMIN")) {
                ...
                int choice = readInt("choose an option: ");
                switch (choice) {
                    case 1 : displayUserAccounts(user) ; break;
                    case 2 : displayUserTransactions(user) ; break;
                    case 3: return;
                    default: System.out.println("Invalid choice");
                }
            }
    ...
    public void loadDefaultUsers() {
        users[userCount++] = new User("admin", "@11adminyami", 512, "ADMIN");
        users[userCount++] = new User("leo", "leo123", 9812, "CUSTOMER");
    }
    ...
    public void displayUserAccounts(User user) {
        ...
        for(int i = 0; i < accountCount; i++) {
            Account a = accounts[i];
            if(a != null && a.getCliCode() == user.getClientCode()) {
                System.out.printf("Account %d | Balance: %.2f | Opened: %s%n",
                   a.getAccCode(), a.getBalance(), a.getOpendate());
            }
        }
    }
